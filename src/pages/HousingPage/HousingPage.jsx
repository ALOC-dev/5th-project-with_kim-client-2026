...
export default function HousingPage({ isAuthenticated, userId, onRequireLogin, onLogout }) {
  ...
  const { preferences, savePreferences, shouldShowOnboarding } = useUserPreferences(userId, isAuthenticated);

  useEffect(() => {
    if (shouldShowOnboarding) setOnboardingMode('all');
  ...
  const content = activePage === 'home' ? homeContent : activePage === 'detail' && selectedListing ? <ListingDetails listing={selectedListing} isFavorite={favorites.includes(selectedListing.id)} onBack={() => setActivePage('home')} onFavorite={handleFavorite} onInquiry={handleInquiry} /> : activePage === 'favorites' ? <FavoritesSection listings={listings} favorites={favorites} compareIds={compareIds} onSelect={openDetail} onFavorite={handleFavorite} onCompare={handleCompare} /> : activePage === 'market' ? <MarketAnalysis listings={listings} /> : activePage === 'checklist' ? <ChecklistSection /> : <ProfileSection preferences={preferences} onOpenBuildingSettings={() => openOnboarding('building')} onOpenBudgetSettings={() => openOnboarding('budget')} />;

  return <main className="housing-page"><Sidebar activePage={activePage === 'detail' ? 'home' : activePage} onNavigate={(page) => { if (!isAuthenticated && page !== 'home') return onRequireLogin(); setActivePage(page); setSelectedListing(null); }} onOpenOnboarding={() => isAuthenticated ? openOnboarding() : onRequireLogin()} /><div className="housing-page__main">{content}</div>{!filterOpen && <ChatAssistant />}{filterOpen && <FilterPanel filters={filters} onChange={handleFilterChange} onClose={() => setFilterOpen(false)} onReset={resetFilters} count={shownListings.length} />}{onboardingMode && <OnboardingSection mode={onboardingMode} preferences={preferences} onClose={closeOnboarding} onDefer={deferOnboarding} onSave={savePreferences} />}</main>;
}
