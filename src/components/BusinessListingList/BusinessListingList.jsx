    ...
    <div className="business-list__cards">
      {listings.map((listing) => <article className="business-list__card" key={listing.id}>
        ...
        <div className="business-list__content">
          <div className="business-list__card-top"><div><h3>{listing.title || '이름 없는 매물'}</h3><p>{listing.address || '주소를 입력해 주세요.'} · {listing.leaseType === 'JEONSE' ? `전세 ${Number(listing.deposit).toLocaleString()}만원` : `월세 ${Number(listing.deposit).toLocaleString()}/${Number(listing.monthlyRent).toLocaleString()}`}{listing.builtYear ? ` · ${listing.builtYear}년 건축` : ''}</p></div><span className={`business-list__status ${listing.status === '학생 노출 중' ? 'is-live' : ''}`}>{listing.status}</span></div>
